...
export async function statusCommand(dest: string) {
  ...
  if (!existsSync(statePath)) {
    ...
    if (pending || (await readRegistry()).scopes[dest]) console.log(`LocalStack setup has pending uninstall items in ${dest}; run localstack uninstall --dry-run with the same scope to review them.`);
    else console.log(`LocalStack setup is not configured in ${dest}`);
    return;
  }
